...
namespace Physics_2D_Library.Tests.SortingTests;

public class RotationClassSorting {
    [Fact]
    public void RotationClassSortsCorrectly() {
        ...
        for (int i = 0; i < answer.Length; i++) {
            rotations[i].Equals(answer[i]).Should().BeTrue();
        }
    }
}
public class PointClassSorting {
    [Fact]
    public void IntegerPointClassSorting() {
        Point2D[] points = {
            new Point2D(6, 7),
            new Point2D(8, 5),
            new Point2D(-2, 5),
        ...
        for (int i = 0; i < answer.Length; i++) {
            points[i].Equals(answer[i]).Should().BeTrue();
        }
    }

    [Fact]
    public void DoublePointClassSorting() {
        Point2D[] points = {
...
public class EquationSorting {
    [Fact]
    public void IntegerEquationSorting() {
        ...
        for (int i = 0; i < answer.Length; i++) {
            equations[i].Equals(answer[i]).Should().BeTrue();
        }
    }

    [Fact]
    public void DoubleEquationSorting() {
        Equation2D[] equations = {
...
public class LineSorting {
    [Fact]
    public void IntegerLineSorting() {
        Line2D[] lines = {
            new Line2D(2, -1, 5, 0),
            new Line2D(-4, -3, -2, 3),
            new Line2D(2, 3, 4, 1),
        ...
        for (int i = 0; i < answer.Length; i++) {
            lines[i].Equals(answer[i]).Should().BeTrue();
        }
    }
}
public class VectorSorting {
    [Fact]
    public void FullSorting() {
        Vector2D[] momentum = {
        ...
        for (int i = 0; i < answer.Length; i++) {
            momentum[i].Equals(answer[i]).Should().BeTrue();
        }
    }
}
public class VectorObjectSorting {
    ...
    public void FullSorting() {
        ...
        for (int i = 0; i < answer.Length; i++) {
            objects[i].Equals(answer[i]).Should().BeTrue();
        }
    }
}
